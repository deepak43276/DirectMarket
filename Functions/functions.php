 <?php
        ...
        while ($rows = mysqli_fetch_array($run_query)) {
            ...
            echo "
                ...
                <div class='card pb-1 pl-1 pr-1 pt-0' style='height:542px'>
                    <br>
                    <div class='mt-0'><b>
                            <h4><img src='iconsmall.png' style='width: 28px; margin-bottom:  10px;'> 
                        </b></h4>
                    </div>
    ...
    {
        global $con;
        $query = "select * from products where product_cat = 3 and not (product_image = '') order by RAND() LIMIT 0,8";
        $run_query = mysqli_query($con, $query);
        while ($rows = mysqli_fetch_array($run_query)) {
            $product_id = $rows['product_id'];
            $product_title = $rows['product_title'];
